CREATE OR REPLACE FUNCTION fn_sync_idatum_cri(in doc_number int8, in cridate text, in remarks text, in fromcontractorcode text, in materialname text, in materialcode text, in uom text, in stockcondition text, in quantity int4, in serialno text, in "type" text)
  RETURNS void AS 
$BODY$ 
DECLARE 
...
if $11='serialized' then
    ...
    if stockid is not null and exists(select 1 from se_stock where stock_rid=stockid and mat_rid=matid) then
    ...
    select mm_rid::int4 into matid from se_material_master where code=materialcode;

    select contact_rid::int4 into contrid from se_contact where vendor_code=fromcontractorcode;

    select stock_rid,mat_rid into stockid, matid from se_stock where mat_rid=matid and holder_rid=contrid;
