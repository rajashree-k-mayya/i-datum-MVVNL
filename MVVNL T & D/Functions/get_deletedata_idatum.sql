CREATE OR REPLACE FUNCTION get_deletedata_idatum()
  RETURNS void AS 
$BODY$ 
BEGIN
with swapcte as(
...
select distinct m.distnid,m.distcode,jsonb_build_object('serialNumber',upper(m.meternum),'miDeleteFlag',true)
,m.remarks,108,m.pushlog_id

from micte m
join tblidatum_updatemi_logs u on u.request::jsonb->>'serialNumber' =upper(m.meternum) and u.status='true'
where not exists(select 1 from tblidatum_consumerswap_logs where request::jsonb->>'serialNumber'=upper(m.meternum) and distnid=m.distnid and description='delete' and status='false')
